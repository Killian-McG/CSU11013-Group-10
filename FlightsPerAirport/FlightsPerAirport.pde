...
class Flight {
  String date, carrier, origin, originCity, originState;
  String dest, destCity, destState;
  int    flightNum, depTime, arrTime;
  ...
  String toString() {
    return carrier + flightNum + " | " + origin + " -> " + dest + " | " + date;
  }
}
...
void setup() {
  size(1200, 650);
  textFont(createFont("SansSerif", LABEL_SIZE));
...
void computeCounts() {
  ...
  for (Flight f : flights) {
    ...
    map.put(f.dest,   map.getOrDefault(f.dest,   0) + 1);
  }
